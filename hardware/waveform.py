...
class Waveform(Thread):
    ...
    def run(self):
        ...
        self.data = memoryview(header.encode() + data.tobytes() + trailer.encode())
    ...
    def __repr__(self):
        string = '<awg520.Waveform> ' + self.fname
        return string
    ...
    def plot(self, t_scale=1., show=True, color='k'):
        ...
        for p in self.pulse_seq:
            i += float(p.duration)/t_scale
            plt.plot([i,i], [-1,1], 'r--')
        t = np.arange(len(self.wave), dtype=np.single) + 1.0
        t /= t_scale
        plt.step(t, self.wave, color)
        plt.xlim((0,t[-1]))
        plt.ylim((-1,1))
...
class Sequence(BytesIO):
    ...
    def __init__(self, name, **kw):
        ...
        super().__init__()
        self.textio = TextIOWrapper(self)
        ...
        self.header = 'MAGIC 3002\r\nLINES '
        self.textio.write(self.header)
        self.update(trailer=True)        
    
    
    def update(self, trailer=False):
        t = time.time()
        self.textio.seek(len(self.header))
        #self.truncate()
        #write body
        self.textio.write('%i\r\n' % len(self.seq))
        for e in self.seq:
            ...
            ch = ['','']
            for i in [0, 1]:
                if e['wave'][i] is not None: ch[i] = e['wave'][i].fname
            rep = e['repeat']
            wai = e['wait']
            res = e['restart']
            got = e['goto']
        ...
        self.textio.write(self.trailer)
        d = time.time() - t
        self.stats[0] += 1
    ...
    def __repr__(self):
        string = '<awg520.Sequence> ' + self.fname
        return string
    ...
    def save_seq(self, filename=None):
        if filename is None:
            if self.fname.upper().endswidth('.SEQ'):
                filename = self.fname
            else:
                filename = self.fname + '.SEQ'
